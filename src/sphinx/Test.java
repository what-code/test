 ...
 */
public class Test {
	public static void main(String[] argv1) throws SphinxException {
		String[] argv = {"-i","test1","-ph","下雨"};
		//if (argv == null || argv.length < 1) {
//			System.out
//					.print("Usage: java -jar sphinxapi.jar [OPTIONS] query words\n\n");
...
//			System.out
//					.print("-ga, --geoanchor <LATATTR> <LONGATTR> <LAT> <LONG>\n");
//			System.out.print("\t\t\tset anchor for geodistance\n");
//			System.out
//					.print("--select <EXPRS>\tselect the listed expressions only\n");
		...
		if (argv != null)
			for (int i = 0; i < argv.length; i++) {
				...
				if ("-h".equals(arg) || "--host".equals(arg))
					...
					cl.SetGeoAnchor(argv[++i], argv[++i],
							Float.parseFloat(argv[++i]),
							Float.parseFloat(argv[++i]));
		...
		/* print me out */
		System.out.println("Query '" + q + "' retrieved " + res.total + " of "
		...
		for (int i = 0; i < res.matches.length; i++) {
			...
			System.out.print((i + 1) + ". id=" + info.docId + ", weight="
					+ info.weight);
			...
			for (int a = 0; a < res.attrNames.length; a++) {
				...
				if ((res.attrTypes[a] & SphinxClient.SPH_ATTR_MULTI) != 0) {
					...
					default:
						System.out.print("(unknown-attr-type="
								+ res.attrTypes[a] + ")");
					}
				}
			}
